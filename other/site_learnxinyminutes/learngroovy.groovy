...
// Interpolation is achieved using the ${} syntax in double quotes or use $ for simple dotted expressions

// Dotted expressions are expressions of the form a.b or a.b.c. 
// Expressions that would contain parentheses like method calls, curly braces for closures, 
// or arithmetic operators, are not dotted expressions and you should use ${}. We recommend 
// always using the ${} notation.
...
// if you a reference to a property name that doesn’t appear to exist, it might be invoking a getter method.
// arguments types are optional
def getSomeValue() {
    return "got it"
    //The return statement is optional in a Groovy method. 
    // The value of the last expression evaluated is returned by default
}

assert "got it" == someValue


// Closure (similar to list comprehension or lambda functions)
def list [1, 2, 3, 4]
list.each {num ->
    println num
}
def greaterThan50(nums) {
    ...
    nums.findAll {
    	// by default "it". Can also use the -> operator
        it > 50
    }
}
...
// it is idiomatic groovy to instead of:
pickEven(10 {
    println it
})
