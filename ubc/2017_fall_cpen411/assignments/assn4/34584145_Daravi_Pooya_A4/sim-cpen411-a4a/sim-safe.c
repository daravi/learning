...
static counter_t g_icache_miss_ai;
...
sim_reg_stats(struct stat_sdb_t *sdb) {
  ...
  /* cache miss rates for part a */
  stat_reg_counter(sdb, "sim_num_icache_miss_ai",
                   "total number of instruction cache misses",
                   &g_icache_miss_ai, g_icache_miss_ai, NULL);
  stat_reg_formula(sdb, "sim_icache_miss_rate_ai",
                   "instruction cache miss rate (percentage) for part (i)",
                   "100*(sim_num_icache_miss_ai / sim_num_insn)", NULL);
  stat_reg_counter(sdb, "sim_num_icache_miss_aii",
                   "total number of instruction cache misses",
                   &g_icache_miss_aii, g_icache_miss_aii, NULL);
  stat_reg_formula(sdb, "sim_icache_miss_rate_aii",
                   "instruction cache miss rate (percentage) for part (ii)",
                   "100*(sim_num_icache_miss_aii / sim_num_insn)", NULL);
...
void create_cache( struct cache *c, unsigned csize, unsigned cways, unsigned cbsize) {
  c->m_ways = cways;
  unsigned total_lines = csize / (cbsize * cways);
  c->m_line_array = (struct block **) calloc( sizeof(struct block *), total_lines );
  unsigned int idx;
  for ( idx = 0; idx < total_lines; idx++ )
    c->m_line_array[idx] = (struct block *) calloc( sizeof(struct block), cways );
  c->m_total_lines = total_lines;
  unsigned offset_bits = log2(cbsize);
  c->m_set_shift = offset_bits;
  unsigned index_bits = log2(total_lines);
  c->m_set_mask = (1 << index_bits) - 1;
  c->m_tag_shift = offset_bits + index_bits;
}

void cache_access( struct cache *c, unsigned addr, counter_t *miss_counter ) {

  unsigned idx, tag, pos, i;
  /* idx: line index within the cache */
  idx = (addr >> c->m_set_shift)&c->m_set_mask;
...
sim_main(void) {
  ...
  /* create the two caches for parts a(i) and a(ii) */
  struct cache *icache_ai = (struct cache *) calloc( sizeof(struct cache), 1 );
  create_cache(icache_ai, 32 * 1024, 1, 64); // direct map

  struct cache *icache_aii = (struct cache *) calloc( sizeof(struct cache), 1 );
  create_cache(icache_aii, 32 * 1024, 4, 32);
  ...
  while (TRUE) {
    ...
    cache_access(icache_ai, regs.regs_PC, &g_icache_miss_ai);
    cache_access(icache_aii, regs.regs_PC, &g_icache_miss_aii);
