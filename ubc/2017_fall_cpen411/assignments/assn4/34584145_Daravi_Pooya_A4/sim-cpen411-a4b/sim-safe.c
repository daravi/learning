...
void cache_access( struct cache *c, unsigned addr, counter_t *miss_counter, counter_t *wb_counter ) {
...
sim_main(void) {
  ...
  create_cache(dcache, 32 * 1024, 8, 64);
...
#include "machine.def"
    ...
    if (MD_OP_FLAGS(op) & F_MEM) {
      ...
      if (MD_OP_FLAGS(op) & F_STORE) {
        is_write = TRUE;
        sim_num_store++;
        cache_access(dcache, data_addr, &g_dcache_miss_st, &g_dcache_wb);
      } else if (MD_OP_FLAGS(op) & F_LOAD) {
        sim_num_load++;
        cache_access(dcache, data_addr, &g_dcache_miss_ld, &g_dcache_wb);
      }
    }
