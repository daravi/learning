...
void cache_access( struct cache *c, enum CacheAccessMode cam, unsigned addr, counter_t *miss_counter, counter_t *wb_counter, struct cache *rc ) {
  ...
  if ((cam == READ) | (cam == WRITE)) {
    if (cache_hit(c, idx, tag, &pos)) {
      /* cache hit */
      if (c->parent) {
        cache_update(c, idx, pos, DELETE, 0, &evicted_tag);
        cache_access(c->parent, UPDATE, addr, miss_counter, wb_counter, rc);
      } else {
        cache_update(c, idx, pos, CHECKOUT, 0, &evicted_tag);
      }
    } else {
      /* cache miss */
      if (c->child) {
        cache_access(c->child, READ, addr, miss_counter, wb_counter, rc);
      } else {
        (*miss_counter)++;
        /* value is loaded to root cache (closest to program) */
        cache_access(rc, UPDATE, addr, miss_counter, wb_counter, rc);
      }
    }
  } else if (cam == UPDATE) {
    if (cache_full(c, idx, &pos)) {
      cache_update(c, idx, pos, REPLACE, tag, &evicted_tag);
      if (c->child) {
        cache_access(c->child, UPDATE, evicted_tag, miss_counter, wb_counter, rc);
      } else {
        if (wb_counter)
          (*wb_counter)++;
      }
    } else {
      cache_update(c, idx, pos, INSERT, tag, &evicted_tag);
    }
  }
  return;
}
